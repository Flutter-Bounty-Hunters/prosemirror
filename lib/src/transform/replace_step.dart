...
class ReplaceStep extends Step {
  ...
  Step? map(Mappable mapping) {
    ...
    final from = this.from == this.to && ReplaceStep.mapBias < 0 ? to : mapping.mapResult(this.from, 1);
  ...
  @override
  Object? toJSON() {
    final json = <String, Object?>{"stepType": "replace", "from": from, "to": to};
    if (slice.size != 0) {
...
class ReplaceAroundStep extends Step {
  ...
  ReplaceAroundStep(this.from, this.to, this.gapFrom, this.gapTo, this.slice, this.insert, [this.structure = false]);
  ...
  @override
  StepResult apply(Node doc) {
    if (structure && (_contentBetween(doc, from, gapFrom) || _contentBetween(doc, gapTo, to))) {
  ...
  @override
  StepMap getMap() {
    return StepMap([from, gapFrom - from, insert, gapTo, to - gapTo, slice.size - insert]);
  }
  ...
  Step? map(Mappable mapping) {
    ...
    final to = mapping.mapResult(this.to, -1);
    final gapFrom = this.from == this.gapFrom ? from.pos : mapping.map(this.gapFrom, -1);
    final gapTo = this.to == this.gapTo ? to.pos : mapping.map(this.gapTo, 1);
    if ((from.deletedAcross && to.deletedAcross) || gapFrom < from.pos || gapTo > to.pos) {
      return null;
    }
    return ReplaceAroundStep(from.pos, to.pos, gapFrom, gapTo, slice, insert, structure);
  }
...
bool _contentBetween(Node doc, int from, int to) {
  ...
  while (distance > 0 && depth > 0 && $from.indexAfter(depth) == $from.node(depth).childCount) {
    depth--;
    distance--;
  }
  if (distance > 0) {
