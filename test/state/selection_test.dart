...
void main() {
  group("Selection >", () {
    test("should follow changes", () {
      final state = TestState(doc: doc(p("hi")), schema: schema);
    ...
    test("moves after an inserted leaf node", () {
      final state = TestState(doc: doc(p("foobar")), schema: schema);
      state.textSel(4);
      state.apply(
        state.tr.replaceSelectionWith(schema.node("horizontal_rule")),
      );
      expect(eq(state.doc, doc(p("foo"), hr(), p("bar"))), isTrue);
      expect(state.selection.head, 7);
      state.textSel(10);
      state.apply(
        state.tr.replaceSelectionWith(schema.node("horizontal_rule")),
      );
      expect(eq(state.doc, doc(p("foo"), hr(), p("bar"), hr())), isTrue);
    ...
    test("allows typing over a leaf node", () {
      ...
      expect(eq(state.doc, doc(p("a"), p("x"), p("b"))), isTrue);
      expect(state.selection.head, 5);
      expect(state.selection.anchor, 5);
    });
    ...
    test("allows deleting a selected block", () {
      ...
      expect(
        eq(state.doc, doc(ul(li(p("bar")), li(p("baz")), li(p("quux"))))),
        isTrue,
      );
      expect(state.selection.head, 3);
      state.nodeSel(2);
      state.deleteSelection();
      expect(eq(state.doc, doc(ul(li(p("baz")), li(p("quux"))))), isTrue);
      expect(state.selection.head, 3);
      state.nodeSel(9);
      state.deleteSelection();
      expect(eq(state.doc, doc(ul(li(p("baz"))))), isTrue);
    ...
    test("preserves the marks of a deleted selection", () {
      final state = TestState(doc: doc(p("foo", em("<a>bar<b>"), "baz")));
      state.deleteSelection();
      expect(state.state.storedMarks!.length, 1);
    });

    test("doesn't preserve non-inclusive marks of a deleted selection", () {
      final state = TestState(doc: doc(p("foo", a(em("<a>bar<b>")), "baz")));
      state.deleteSelection();
      expect(state.state.storedMarks!.length, 1);
    });

    test(
      "doesn't preserve marks when deleting a selection at the end of a block",
      () {
        final state = TestState(doc: doc(p("foo", em("bar<a>")), p("b<b>az")));
        state.deleteSelection();
    ...
    test(
      "drops non-inclusive marks at the end of a deleted span when appropriate",
      () {
        final state = TestState(
          doc: doc(p("foo", a("ba", em("<a>r<b>")), "baz")),
        );
        state.deleteSelection();
        expect(
          state.state.storedMarks!.map((mark) => mark.type.name).join(","),
          "em",
        );
      },
    );
    ...
    test("keeps non-inclusive marks when still inside them", () {
      final state = TestState(
        doc: doc(p("foo", a("b", em("<a>a<b>"), "r"), "baz")),
      );
      state.deleteSelection();
    ...
    test("preserves marks when typing over marked text", () {
      ...
      expect(eq(state.doc, doc(p("foo ", em("quux"), " baz"))), isTrue);
      state.apply(state.tr.insertText("bar", 5, 9));
      expect(eq(state.doc, doc(p("foo ", em("bar"), " baz"))), isTrue);
    });
    ...
    test("allows deleting a leaf", () {
      ...
      state.nodeSel(3);
      state.deleteSelection();
      expect(eq(state.doc, doc(p("a"), hr(), p("b"))), isTrue);
      expect(state.selection.from, 3);
      state.deleteSelection();
      expect(eq(state.doc, doc(p("a"), p("b"))), isTrue);
    ...
    test("properly handles deleting the selection", () {
      ...
      expect(
        eq(state.doc, doc(p("foobar"), blockquote(p("hi")), p("ay"))),
        isTrue,
      );
      expect(state.selection.head, 4);
      state.nodeSel(9);
      state.apply(state.tr.deleteSelection());
      expect(eq(state.doc, doc(p("foobar"), p("ay"))), isTrue);
      expect(state.selection.from, 9);
      state.nodeSel(8);
      state.apply(state.tr.deleteSelection());
      expect(eq(state.doc, doc(p("foobar"))), isTrue);
    ...
    test("can replace inline selections", () {
      ...
      expect(eq(state.doc, doc(p("foo", br(), "bar", img(), "baz"))), isTrue);
      expect(state.selection.head, 5);
      expect(state.selection.empty, isTrue);
      state.nodeSel(8);
      state.apply(state.tr.insertText("abc"));
      expect(eq(state.doc, doc(p("foo", br(), "barabcbaz"))), isTrue);
      expect(state.selection.head, 11);
      expect(state.selection.empty, isTrue);
      state.nodeSel(0);
      state.apply(state.tr.insertText("xyz"));
      expect(eq(state.doc, doc(p("xyz"))), isTrue);
    });
    ...
    test("can replace a block selection", () {
      ...
      expect(
        eq(state.doc, doc(p("abc"), pre(), hr(), blockquote(p("ow")))),
        isTrue,
      );
      expect(state.selection.from, 7);
      state.nodeSel(8);
      state.apply(state.tr.replaceSelectionWith(schema.node("paragraph")));
      expect(eq(state.doc, doc(p("abc"), pre(), hr(), p())), isTrue);
      expect(state.selection.from, 9);
    });

    test(
      "puts the cursor after the inserted text when inserting a list item",
      () {
        final state = TestState(doc: doc(p("<a>abc")));
        final source = doc(ul(li(p("<a>def<b>"))));
        state.apply(
          state.tr.replaceSelection(
            source.slice(source.tag["a"]!, source.tag["b"]!, true),
          ),
        );
  ...
  group("TextSelection.between >", () {
    test("uses arguments when possible", () {
      final document = doc(p("f<a>o<b>o"));
      final selection = TextSelection.between(
        document.resolve(document.tag["b"]!),
        document.resolve(document.tag["a"]!),
      );
      expect(selection.anchor, document.tag["b"]);
      expect(selection.head, document.tag["a"]);
    });

    test("will adjust when necessary", () {
      final document = doc("<a>", p("foo"));
      final selection = TextSelection.between(
        document.resolve(document.tag["a"]!),
        document.resolve(document.tag["a"]!),
      );
    ...
    test("uses bias when adjusting", () {
      final document = doc(p("foo"), "<a>", p("bar"));
      final pos = document.resolve(document.tag["a"]!);
    ...
    test("will fall back to a node selection", () {
      final document = doc(hr, "<a>");
      final selection = TextSelection.between(
        document.resolve(document.tag["a"]!),
        document.resolve(document.tag["a"]!),
      );
      expect((selection as NodeSelection).node, document.firstChild);
    });

    test("will collapse towards the other argument", () {
      final document = doc("<a>", p("foo"), "<b>");
