...
void main() {
  group("ChangeSet >", () {
    ...
    test("merges adjacent canceling edits", () {
      _find(document(p("hello")), [(tr) => tr.delete(3, 5).insert(3, _text("ll"))], []);
    });
    ...
    test("merges delete/insert from different addStep calls", () {
      _find(document(p("hello")), [(tr) => tr.delete(3, 5), (tr) => tr.insert(3, _text("ll"))], []);
    });

    test("revert a deletion by inserting the character again", () {
      _find(
        document(p("bar")),
        [(tr) => tr.delete(2, 3), (tr) => tr.insert(2, _text("x")), (tr) => tr.insert(2, _text("a"))],
        [
          [3, 3, 3, 4],
        ],
      );
    ...
    test("insert character before changed character", () {
      _find(
        document(p("bar")),
        [(tr) => tr.delete(2, 3), (tr) => tr.insert(2, _text("x")), (tr) => tr.insert(2, _text("x"))],
        [
          [2, 3, 2, 4],
        ],
      );
    ...
    test("merges insert/delete from different addStep calls", () {
      _find(document(p("ok")), [(tr) => tr.insert(2, _text("--")), (tr) => tr.delete(2, 4)], []);
    });
    ...
    test("can incrementally undo then redo", () {
      _find(
        document(p("bar")),
        [(tr) => tr.delete(2, 3), (tr) => tr.insert(2, _text("a")), (tr) => tr.delete(2, 3)],
        [
          [2, 3, 2, 2],
        ],
      );
    ...
    test("can map through complicated changesets", () {
      _find(
        document(p("12345678901234")),
        [
          (tr) => tr.delete(9, 12).insert(6, _text("xyz")).replaceWith(2, 3, _text("uv")),
    ...
    test("doesn't get confused by split deletions", () {
      _find(
        document(blockquote(h1("one"), p("two four"))),
        [(tr) => tr.delete(7, 11), (tr) => tr.replaceWith(0, 13, blockquote(h1("one"), p("four")))],
        [
          [
            7,
            11,
    ...
    test("doesn't get confused by multiply split deletions", () {
      _find(
        document(blockquote(h1("one"), p("two three"))),
        [
          (tr) => tr.delete(14, 16),
          (tr) => tr.delete(7, 11),
          (tr) => tr.delete(3, 5),
...
void _find(Node document, List<void Function(Transform)> builds, List<Object> changes, [Object? separator]) {
  ...
  for (var index = 0; index < builds.length; index++) {
    ...
    set = set.addSteps(transform.doc, transform.mapping.maps, _dataFor(separator, index));
    currentDocument = transform.doc;
  }

  final owner = separator != null && changes.isNotEmpty && (changes[0] as List).length > 4;
