...
void main() {
  group("Rebasing collaborative steps >", () {
    test("supports concurrent typing", () {
      _rebaseAllOrders(document(p("h<1>ell<2>o")), [
        (transform) => _type(transform, 2, "X"),
        (transform) => _type(transform, 5, "Y"),
      ], document(p("hX<1>ellY<2>o")));
    });
    ...
    test("supports multiple concurrently typed characters", () {
      ...
      ], document(p("hXYZ<1>ellUV<2>o")));
    });

    test("supports three concurrent typers", () {
      _rebaseAllOrders(document(p("h<1>ell<2>o th<3>ere")), [
        (transform) => _type(transform, 2, "X"),
        (transform) => _type(transform, 5, "Y"),
        (transform) => _type(transform, 9, "Z"),
      ], document(p("hX<1>ellY<2>o thZ<3>ere")));
    });

    test("handles wrapping of changed blocks", () {
      _rebaseAllOrders(document(p("<1>hell<2>o<3>")), [
        (transform) => _type(transform, 5, "X"),
        (transform) => _wrap(transform, 1, "blockquote"),
      ], document(blockquote(p("<1>hellX<2>o<3>"))));
    });

    test("handles insertions in deleted content", () {
      _rebaseAllOrders(document(p("hello<1> wo<2>rld<3>!")), [
    ...
    test("allows deleting the same content twice", () {
      _rebaseInOrder(document(p("hello<1> wo<2>rld<3>!")), [
        (transform) => transform.delete(6, 12),
        (transform) => transform.delete(6, 12),
      ], document(p("hello<3>!")));
    });
    ...
    test("isn't confused by joining a block that's being edited", () {
      _rebaseAllOrders(document(ul(li(p("one")), "<1>", li(p("tw<2>o")))), [
        (transform) => _type(transform, 12, "A"),
        (transform) => transform.join(8),
      ], document(ul(li(p("one"), p("twA<2>o")))));
    });
        ...
        [
          (transform) {
            return transform.replace(
              transform.doc.tag["1"]!,
              transform.doc.tag["3"]!,
              document(p("a"), blockquote(p("b")), p("c")).slice(2, 9),
            );
          },
          (transform) => _type(transform, transform.doc.tag["2"]!, "ayay"),
        ],
        document(p("b<before>efore"), blockquote(ul(li(p("o"), blockquote(p("b")), p("<3>ee")))), p("a<after>fter")),
      );
    });

    test("maps through inserts", () {
      _rebaseAllOrders(document(p("X<1>X<2>X")), [
        (transform) => _type(transform, 2, "hello"),
        (transform) => _type(transform, 3, "goodbye").delete(4, 7),
      ], document(p("Xhello<1>Xgbye<2>X")));
    });

    test("handles concurrent removal of blocks", () {
      _rebaseInOrder(document(p("a"), "<1>", p("b"), "<2>", p("c")), [
        (transform) => transform.delete(transform.doc.tag["1"]!, transform.doc.tag["2"]!),
        (transform) => transform.delete(transform.doc.tag["1"]!, transform.doc.tag["2"]!),
      ], document(p("a"), "<2>", p("c")));
    });

    test("discards edits in removed blocks", () {
      _rebaseAllOrders(document(p("a"), "<1>", p("b<2>"), "<3>", p("c")), [
        (transform) => transform.delete(transform.doc.tag["1"]!, transform.doc.tag["3"]!),
        (transform) => _type(transform, transform.doc.tag["2"]!, "ay"),
      ], document(p("a"), "<3>", p("c")));
    });

    test("preserves double block inserts", () {
      _rebaseInOrder(document(p("a"), "<1>", p("b")), [
        (transform) => transform.replaceWith(3, 3, schema.node("paragraph")),
        (transform) => transform.replaceWith(3, 3, schema.node("paragraph")),
      ], document(p("a"), p(), p(), "<1>", p("b")));
    });
  });
}
...
void _rebaseInOrder(Node document, List<_TransformAction> clients, Node expected) {
  _runRebase(clients.map((client) => client(Transform(document))).toList(), expected);
}

void _rebaseAllOrders(Node document, List<_TransformAction> clients, Node expected) {
  final transforms = clients.map((client) => client(Transform(document))).toList();
...
void _runRebase(List<Transform> transforms, Node expected) {
  ...
  for (final transform in transforms) {
    ...
    final firstRebasedStep = transform.steps.length + fullTransform.steps.length;
    rebaseSteps(_rebaseableSteps(transform), fullTransform.steps, rebased);
    for (var stepIndex = firstRebasedStep; stepIndex < rebased.steps.length; stepIndex++) {
...
void _expectTagsMapped(Node startDocument, Transform fullTransform, Node expected) {
...
Transform _wrap(Transform transform, int position, String typeName) {
  ...
  return transform.wrap(resolvedPosition.blockRange()!, [(type: schema.nodes[typeName]!, attrs: null)]);
}
